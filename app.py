...
# Configurar las carpetas que el .py va a afectar. En estas se suben la imagen que se va a meter al modelo, y el resultado de la predicción. Tambien se establecen las extensiones aceptadas de las imagenes a subir.
...
# Cargar el modelo (entrenado en google colab)
model = YOLO('model.pt')

# Mappeo de la comida. La columna en la izquierda son las clases del dataset utilizado, el cual es originalmente en ingles. La derecha es esto mismo en español, ya que este es el lenguaje que vamos a usar en nuestra pagina. 
# Asi, los resultados son alamcenados en español 
FOOD_MAPPING = {
    'almond': 'almendra',
    'apple': 'manzana',
    'asparagus': 'espárrago',
...
# Información de las recetas. Aqui estan todas las recetas, incluyendo los ingredientes, los cuales vienen de las clases del modelo.
# Cada integrante esta al menos en una receta, por lo que siempre mostrara algun resultado.
RECIPE_DATABASE = [
    {
...
#Aqui se buscan las recetas que apliquen en base a los ingredientes detectados. Estos pueden ser de dos tipos: completos y parcialmente completos.
#Completos: Se tienen todos los ingredientes. Parciales: Se tienen algunos ingredientes.
#Se añade las recetas parciales para siempre poder mostrar una respuesta independiente de cual es sean los elementos de la lista.
...
def find_matching_recipes(available_ingredients):
    ...
    for recipe in RECIPE_DATABASE:
        ...
        if matching_ings:
            # Se ve cuales son las recetas completas
            ...
            # qui las parciales
                ...
                recipe['matching_ings'] = matching_ings  
...
def upload_file():
    ...
    # Crea una lista vacia para la lista de ingredientes, en caso de que no exista 
    if 'detected_foods' not in session:
        session['detected_foods'] = []
    
    if request.method == 'POST':
        # Subir la imagen desde los archivos del dispositivo
        if 'file' in request.files:
            ...
            if file.filename != '' and file and allowed_file(file.filename):
                ...
                # Se guarda la imagen resultante tras la predicción.
                result_filename = f"result_{filename}"
                result_path = os.path.join(app.config['RESULT_FOLDER'], result_filename)
                Image.fromarray(results[0].plot()[..., ::-1]).save(result_path)
                
                # Extrae las clases detectadas en la imagen, y las pasa a la lista del usuario
        ...
        # Modificaciones que el usuario puede hacer a la lista, como elimienar un ingrediente, o añadirlo manualmente,
...
#Ruta de Flask para que el usuario pueda borrar todos los elementos de la lista.
...
#Ruta de Flask para que se muestre las recetas que aplican, en base a la lista de ingredientes
@app.route('/recipes')
def show_recipes():
...
#Corre el servidor flask y verifica la existencia de las carpetas requeridas
